...
const MainButtons: React.FC<MainButtonsProps> = ({ contactData, repoUrl }) => {
  ...
  const { repo} = getGHCredentials(repoUrl);

  const ghButtons: GHButtonProps[] = [
    // Follow button
    {
      resource: {
        endpoint: `https://api.github.com/users/sagarjsr`,
        attr: 'followers',
      },
      href: `https://github.com/sagarjsr`,
      title: `Follow @sagarjsr on GitHub`,
      icon: ['fab', 'github'],
      size: 'lg',
      text: `Follow @sagarjsr`,
    },
    // Stargazers button
    {
      resource: {
        endpoint: `https://api.github.com/repos/sagarjsr/${repo}`,
        attr: 'stargazers_count',
      },
      href: repoUrl,
      title: `Star sagarjsr/${repo} on GitHub`,
      icon: 'star',
      size: 'sm',
      text: 'Star',
    },
